...
function ChatPage() {
  ...
  useEffect(() => {
    ...
    socket.on("seenAll", (data) => {
      for (let i = 0; i < messages.length; i++) {
        messages[i].seen =
          data.chat == (localStorage.getItem("chatId") || 1) ? true : msg.seen;
      }
    });
  ...
  useEffect(() => {
    ...
    if (lastMessages.length !== 0) {
      for (
        let i = animateFrom.current;
        i < content_panel_children.length;
        i++
      ) {
        scrollBy +=
          content_panel_children[i].getBoundingClientRect().height * 1000;
  ...
  function sendMessage() {
    ...
    for (let i = 0; i < splitted.length - 1; i++) {
      if (
        validateString(splitted[i], "username", 1, 64) &&
        customEmojis.find((emoji) => emoji.name === splitted[i])
      ) {
        splitted[i] =
          `![${splitted[i]}](${address}/emojis/${customEmojis.find((emoji) => emoji.name === splitted[i]).id}.webp)`;
      }
    }
  ...
  function openChat(username) {
    ...
    document.getElementById("top_panel").style.borderRadius =
      "2svh 2svh 2svh 2svh";
    document.getElementById("content_panel").style.borderRadius =
      "2svh 2svh 2svh 2svh";
    document.getElementById("input_panel").style.borderRadius =
      "2svh 2svh 2svh 2svh";
    ...
    Array.from(document.getElementById("input_panel").children).forEach(
      (el) => {
        el.style.translate = "0 0";
      },
    );
    ...
    fetch(`${address}/attach`, {
      ...
      .then((data) => {
        if (data.success) {
          ...
          input.value +=
            " " +
            data.urls.map((att) => `![attachment](${address}${att})`).join(" ");
          input.focus();
        } else {
          showError(data.msg);
        }
  ...
  return (
    <div>
      <div className="App" id="app">
        <div className="LeftPanel" id="left_panel">
          <div className="ChatsHeader">
            ...
            <button
              className="ChatPlusButton"
              onClick={() => openPopup("create-chat")}
            >
          ...
          <div className="ChatsPanel" id="chats_panel">
            ...
            {chats.map((chat) => (
              <ProfileThing
                ...
                src={`${address}/avatars/${chat.participants.find((el) => el.id != localStorage.getItem("id"))?.id}.webp`}
                key={chat.id}
              />
            ))}
          </div>
        ...
        <div className="RightPanel" id="right_panel">
          ...
          <div
            className={`InputPanel ${lines > 1 ? "expanded" : ""}`}
            id="input_panel"
          >
      ...
      <Popup
        title={<Trans>chat_creation</Trans>}
        name="create-chat"
        scale={0.5}
      >
        <input
